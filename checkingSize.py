...
# Directories
train_directory = './annotations/xmls' # It should contain the xml files with bounding boxes
test_directory = './annotations/xmls' # It should contain the xml files with bounding boxes
boxes_count=0
...
for tree in [train_directory, test_directory]:
    ...
    for file in xmlFiles(tree + '/*.xml'):
       ...
       for box in boxes:
          ...
          if x_value<33 or y_value <33 or int(xmax.text) > 600 or int(ymax.text) > 338 or int(xmax.text) <0 or int(ymax.text) <0:
